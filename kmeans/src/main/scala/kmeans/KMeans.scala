...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    means.map(point => (point, Nil)).toMap ++ points.groupBy(p => findClosest(p, means))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (oldMeans zip newMeans).forall { case (oldPoint, newPoint) => oldPoint.squareDistance(newPoint) <= eta }
