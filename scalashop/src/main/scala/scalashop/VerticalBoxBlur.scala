...
object VerticalBoxBlur {
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    val step = if (numTasks >= dst.width) 1 else dst.width / (dst.width / numTasks)
    val rangeList = 0 to dst.width by step
    ...
    blur(src, dst, rangeList.last, dst.width, radius)
