...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    var maxResult = 0f
    while (index < until) {
      maxResult = max(maxResult, input(index) / index)
      index += 1
    }
    maxResult
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val (left, right) = parallel(
        upsweep(input, from, medium, threshold),
        upsweep(input, medium, end, threshold)
      )
      Node(left, right)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
                          startingAngle: Float, from: Int, until: Int
                         ): Unit = {
    var index = from
    var maxAngle = startingAngle
    while (index < until) {
      maxAngle = max(maxAngle, input(index) / index)
