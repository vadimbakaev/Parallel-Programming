...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    ...
    if (money == 0) 1
      ...
      val list = coins.filter(_ <= money)
      list.map { coin =>
        val newMoneyToCalc = money - coin
        if (newMoneyToCalc == 0) 1 else countChange(money - coin, list.filter(_ >= coin))
      }.sum
    }
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins)) countChange(money, coins)
      ...
      list.par.map { coin =>
        val newMoneyToCalc = money - coin
        if (newMoneyToCalc == 0) 1 else parCountChange(money - coin, list.filter(_ >= coin), threshold)
      }.sum
    }
  }
